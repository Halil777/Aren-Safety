...
@Controller('tenant/project-codes')
export class ProjectCodesController {
  constructor(private readonly storage: StorageService) {}

  private read(tenantId: string) {
    return this.storage.readCollection<ProjectCodeRecord>(tenantId, 'project-codes');
  }
  private write(tenantId: string, docs: ProjectCodeRecord[]) {
    return this.storage.writeCollection<ProjectCodeRecord>(tenantId, 'project-codes', docs);
  }
  ...
  @Post()
  create(@Req() req: any, @Body() dto: CreateProjectCodeDto) {
    const now = new Date().toISOString();
    const doc: ProjectCodeRecord = {
      id: randomUUID(),
      ...dto,
  ...
  @Patch(':id')
  update(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateProjectCodeDto) {
    const all = this.read(req.tenantId);
