            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-border">
                <thead className="bg-muted/50">
                  <tr>
                    <Th>{t('projects.table.name', { defaultValue: 'Name' })}</Th>
                    <Th>{t('projects.table.client', { defaultValue: 'Client' })}</Th>
                    <Th>{t('projects.table.location', { defaultValue: 'Location' })}</Th>
                    <Th>{t('projects.table.head', { defaultValue: 'Project Head' })}</Th>
                    <Th>{t('projects.table.start', { defaultValue: 'Start Date' })}</Th>
                    <Th>{t('projects.table.end', { defaultValue: 'End Date' })}</Th>
                  </tr>
                </thead>
                    ...
                    <tr key={project.id} className="hover:bg-muted/50">
                      <Td>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {project.projectLocation || t('common.noData', { defaultValue: '—' })}
                        </div>
                      </Td>
                      ...
                      <Td>
                        <div className="flex items-center gap-2">
                          <Button variant="outline" size="sm" onClick={() => handleOpen(project)}>
            ...
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <div>
                <h2 className="text-lg font-semibold">
                  {editingProject
                    ? t('projects.form.editTitle', { defaultValue: 'Edit project' })
                    : t('projects.form.createTitle', { defaultValue: 'Create project' })}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {t('projects.form.subtitle', { defaultValue: 'Project details' })}
                </p>
              </div>
            ...
            <div className="h-[calc(100%-64px)] overflow-y-auto p-4">
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  />
                  <Field
                    label={t('projects.form.client', { defaultValue: 'Client' })}
                    value={formState.projectClient}
                    onChange={value => setFormState(s => ({ ...s, projectClient: value }))}
                  />
                </div>

                <div className="grid gap-4 md:grid-cols-2">
                  <Field
                    label={t('projects.form.location', { defaultValue: 'Location' })}
                    value={formState.projectLocation}
                    onChange={value => setFormState(s => ({ ...s, projectLocation: value }))}
                  />
                  <Field
                    label={t('projects.form.head', { defaultValue: 'Project head' })}
                    value={formState.projectHead}
                    onChange={value => setFormState(s => ({ ...s, projectHead: value }))}
                  />
                </div>

                <div className="grid gap-4 md:grid-cols-2">
                  <Field
                    label={t('projects.form.start', { defaultValue: 'Start date' })}
                    type="date"
                    value={formState.startDate}
                    onChange={value => setFormState(s => ({ ...s, startDate: value }))}
                  />
                  <Field
